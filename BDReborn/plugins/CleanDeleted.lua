...
tdcli.getUser(v.user_id_, clean_cb, nil)
    ...
    tdcli.sendMessage(msg.to.id, msg.id, 1, '_کاربران دلیت اکانت شده از گروه حذف شدند_', 1, 'md') 
  end 
  tdcli_function ({ID = "GetChannelMembers",channel_id_ = getChatId(msg.to.id).ID,offset_ = 0,limit_ = 10000}, check_deleted, nil)
  end 
end 
