...
if ((matches[1] == 'rmsg all' and not Clang) or (matches[1] == "پاکسازی همه" and not Clang)) and is_mod(msg) then
...
------------------------------------------
    if ((matches[1]:lower() == "rmsg" and not Clang) or (matches[1] == "پاکسازی" and not Clang)) and is_mod(msg) then
...
------------------------------------------
if ((matches[1]:lower() == "del" and not Clang) or (matches[1] == "حذف" and not Clang))  and msg.reply_to_message_id_ ~= 0 and is_mod(msg) then
        tdcli.deleteMessages(msg.to.id,{[0] = tonumber(msg.reply_id),msg.id})
end
------------------------------------------
if (matches[1]:lower() == "mydel" and not Clang) or (matches[1] == "پاکسازی پیام های من" and not Clang) then  
